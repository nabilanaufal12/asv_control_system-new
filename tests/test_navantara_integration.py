...
class TestNavantaraIntegration(unittest.TestCase):
    ...
    def test_case_1_waypoint_logic(self):
        ...
        self.assertEqual(
            len(self.handler.current_state.waypoints), 0, "Waypoints should be empty"
        )
        ...
        self.handler.process_command(
            "SET_WAYPOINTS",
            {"waypoints": valid_wps, "arena": "Arena_B", "inversion_trigger_wp": 1},
        )
        self.assertEqual(
            len(self.handler.current_state.waypoints), 2, "Should load 2 waypoints"
        )
        self.assertEqual(
            self.handler.current_state.active_arena,
            "Arena_B",
            "Arena should be parsed as Arena_B",
        )
        ...
        self.assertEqual(
            self.handler.current_state.nav_target_wp_index,
            1,
            "Waypoint index should increment",
        )
    ...
    def test_case_2_state_machine_and_serial_payload(self):
        ...
        self.assertEqual(
            self.handler.current_state.control_mode, "AUTO", "Mode should be AUTO"
        )

        # Transition to Vision Target ('A' Mode logic handler)
        self.handler._handle_vision_target_update(
            {"active": True, "obstacle_class": "kotak-hijau"}
        )
        self.assertTrue(
            self.handler.current_state.vision_target["active"],
            "Vision target should be active",
        )
        self.assertEqual(
            self.handler.current_state.vision_target["obstacle_class"], "kotak-hijau"
        )
    ...
    def test_case_3_auto_capture_and_reverse(self):
        ...
        self.assertEqual(
            self.handler.current_state.nav_motor_cmd,
            1000,
            "Should reverse motor/dir to 1000",
        )
    ...
    def test_case_4_websocket_connection_drops(self):
        ...
        try:
            ...
            self.assertFalse(
                self.handler.is_streaming_to_gui, "Should cleanly stop streaming"
            )
