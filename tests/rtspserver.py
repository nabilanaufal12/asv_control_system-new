...
MIN_DEFECT_DEPTH = (
    20 * 256
)  # Kedalaman minimum lekukan (defect) untuk dianggap sebagai pemisah jari
...
if camera_1 is None or camera_2 is None:
    print(
        "Aplikasi dimatikan karena salah satu kamera gagal diinisialisasi. Cek indeks dan izin."
    )
    sys.exit(1)
...
def video_feed_1():
    ...
    return Response(
        generate_frames(camera_1), mimetype="multipart/x-mixed-replace; boundary=frame"
    )
...
def video_feed_2():
    ...
    return Response(
        generate_frames(camera_2), mimetype="multipart/x-mixed-replace; boundary=frame"
    )
