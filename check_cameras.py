...
def find_available_cameras(max_cameras_to_check=5):
    ...
    if not dshow_cameras and not msmf_cameras:
        ...
        print("   Pastikan juga tidak ada aplikasi lain (Zoom, OBS, dll.) yang sedang menggunakan kamera.")
    else:
        ...
        print("\n--- Rekomendasi ---")
        if msmf_cameras:
            print(f"👍 Gunakan salah satu dari indeks berikut di config.json Anda: {msmf_cameras}")
            print("   Jika aplikasi utama masih gagal, modifikasi file vision_service.py")
            print("   untuk menggunakan 'cv2.CAP_MSMF' saat memanggil cv2.VideoCapture.")
        elif dshow_cameras:
            print(f"👍 Gunakan salah satu dari indeks berikut di config.json Anda: {dshow_cameras}")
