...
try:
    ...
    print(
        f"[Server] KRITIS: Gagal memuat config.json. Pastikan file ada di root proyek. Error: {e}"
    )
    exit()
# --- AKHIR PERBAIKAN ---
...
class AsvHandler:
    ...
    def __init__(self, config_data):
        ...
        self.use_simulation = self.config.get("general", {}).get(
        ...
        if self.use_simulation:
            ...
            self._logic_thread = threading.Thread(
                target=self._main_logic_loop, daemon=True
            )
    ...
    def _main_logic_loop(self):
        ...
        while self.running:
            ...
            if (
                self.current_state.get("control_mode") == "AUTO"
                and not self.vision_override_active
            ):
    ...
    def _simulation_loop(self):
        ...
        while self.running:
            ...
            if self.current_state["control_mode"] == "MANUAL":
                speed_ms = (
                    1.0 if "W" in manual_keys else -1.0 if "S" in manual_keys else 0
                )
                turn_input = (
                    1 if "D" in manual_keys else -1 if "A" in manual_keys else 0
                )
                turn_rate = turn_input * 3.0
                self.current_state["status"] = (
                    "MANUAL CONTROL" if manual_keys else "IDLE"
                )
            ...
            self.current_state["heading"] = (
                self.current_state["heading"] + turn_rate
            ) % 360
        ...
        if not waypoints or wp_index >= len(waypoints):
            if not is_simulation:
                self.send_serial_command(
                    f"S{actuator_config.get('motor_pwm_stop',1500)};D{actuator_config.get('servo_default_angle',90)}\n"
                )
            self.current_state["cog"] = self.current_state.get("heading", 0.0)
            return 0.0
        current_lat, current_lon, current_heading = (
            self.current_state.get("latitude", 0.0),
            self.current_state.get("longitude", 0.0),
            self.current_state.get("heading", 0.0),
        )
        target_wp = waypoints[wp_index]
        target_bearing = self._get_bearing_to_point(
            current_lat, current_lon, target_wp["lat"], target_wp["lon"]
        )
        self.current_state["cog"] = target_bearing
        distance_to_wp = self._haversine_distance(
            current_lat, current_lon, target_wp["lat"], target_wp["lon"]
        )
        ...
        if is_simulation:
            ...
            servo_angle = max(
                ...
                min(
                    ...
                    int(
                        servo_default
                        - (turn_error / 90.0) * (servo_default - servo_min)
                    ),
                ),
            )
    ...
    def process_command(self, command, payload):
        if command == "CONFIGURE_SERIAL":
            if payload.get("serial_port") == "AUTO":
                self.find_and_connect_esp32(payload.get("baud_rate"))
            else:
                self.connect_serial(
                    payload.get("serial_port"), payload.get("baud_rate")
                )
            ...
            keys = set(payload)
            actuator_config = self.config.get("actuators", {})
            pwm_stop, pwm_power = actuator_config.get(
                "motor_pwm_stop", 1500
            ), actuator_config.get("motor_pwm_manual_power", 150)
            servo_default, servo_min = actuator_config.get(
                "servo_default_angle", 90
            ), actuator_config.get("servo_min_angle", 45)
            forward, turn = (1 if "W" in keys else -1 if "S" in keys else 0), (
                1 if "D" in keys else -1 if "A" in keys else 0
            )
            motor_pwm = pwm_stop + forward * pwm_power
            servo_angle = max(
                0, min(180, int(servo_default - turn * (servo_default - servo_min)))
            )
            self.send_serial_command(f"S{motor_pwm};D{servo_angle}\n")
            ...
            if self.vision_override_active and payload.get("command"):
                self.send_serial_command(payload.get("command"))
...
def get_status():
    return jsonify(asv_handler.current_state.copy())
