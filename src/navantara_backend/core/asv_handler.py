...
class AsvHandler:
    def __init__(self, config, socketio):
        ...
        self._last_log_reason = "" # <-- [MODIFIKASI] Variabel anti-spam
        ...
        self.current_state = {
            ...
            "gyro_z": 0.0,
            "accel_x": 0.0,
            "rc_channels": [1500] * 6, # Pastikan ini ada
    ...
    def _update_and_emit_state(self):
        if self.running:
            with self.state_lock:
                ...
                # [PERBAIKAN BUG] RC Override sekarang akan berfungsi kembali
                ...
                if rc_mode_switch < 1500:
                    ...
                    if esp_status == "WAYPOINT":
                         state_copy["status"] = (
                            f"WAYPOINT NAVIGATION ({state_copy.get('nav_target_wp_index', 1)}/{total_wps})"
                        )
        ...
        while self.running:
            ...
            if line:
                try:
                    ...
                    if line.strip():
                        # [MODIFIKASI] Tetap nonaktifkan log telemetri yang masuk
                        # print(f"[Serial] Menerima data mentah (Bukan JSON): {line}")
                        pass 
                except Exception as e:
                    print(f"[Serial] Error parsing: {e}, Data: {line}")
    ...
    def _parse_json_telemetry(self, data):
        try:
            with self.state_lock:
                ...
                # [PERBAIKAN BUG] Baca data RC Channel dari JSON
            ...
            print(f"[AsvHandler] Gagal mem-parsing data JSON: {e}. Data: {data}")
    ...
    def main_logic_loop(self):
        ...
        while self.running:
            ...
            # [PERBAIKAN BUG] Nilai ini sekarang akan diupdate dengan benar dari JSON
            rc_mode_switch = state_for_logic.get("rc_channels", [1500] * 6)[4]
            command_to_send = None
            log_reason = None # <-- [MODIFIKASI] Variabel untuk logging anti-spam

            # --- [MODIFIKASI] Logika Logging Anti-Spam untuk SEMUA MODE ---
            
            # PRIORITAS 1: RC OVERRIDE
            if rc_mode_switch < 1500:
                command_to_send = None
                log_reason = "[LOG | RC] RC Manual Override Aktif. Perintah Jetson ditahan."
            
            # PRIORITAS 2: MANUAL GUI (WASD)
            elif state_for_logic.get("control_mode") == "MANUAL":
                command_to_send = None # Perintah dikirim oleh _handle_manual_control
                log_reason = "[LOG | MANUAL] Mode Manual GUI (WASD) Aktif."
            ...
            # PRIORITAS 3: AUTO (AI & WAYPOINT)
                ...
                if self.gate_target.get("active", False):
                    ...
                    if gate_center_x is not None:
                        ...
                        # [MODIFIKASI] Set log_reason (memenuhi permintaan "A" + servo/pwm)
                        log_reason = f"[LOG | AUTO AI] Mengirim [Gate]: {command_to_send.strip()}"
                ...
                # === PRIORITAS 3.2: PENGHINDARAN TUNGGAL BERKONTEKS ===
                    ...
                    if target_side:
                        ...
                        log_reason = f"[LOG | AUTO AI] Mengirim [Avoid Ctx]: {command_to_send.strip()}"
                    else:
                ...
                # === PRIORITAS 3.3: PENGHINDARAN TUNGGAL SEDERHANA (TANPA KONTEKS) ===
                    ...
                    command_to_send = f"A,{servo_cmd},{pwm_cmd}\n"
                    log_reason = f"[LOG | AUTO AI] Mengirim [Avoid]: {command_to_send.strip()}"
                ...
                # === PRIORITAS 3.4: FASE RECOVERY/PELURUSAN SETELAH MENGHINDAR ===
                    ...
                    command_to_send = f"A,{servo_cmd},{pwm_cmd}\n"
                    log_reason = f"[LOG | AUTO AI] Mengirim [Recovery]: {command_to_send.strip()}"
                ...
                # === PRIORITAS 3.5 (DEFAULT): NAVIGASI WAYPOINT ===
                    ...
                    self.last_pixel_error = 0
                    command_to_send = "W\n"
                    # [MODIFIKASI] Set log_reason (memenuhi permintaan "W")
                    log_reason = "[LOG | AUTO WP] Mengirim [Waypoint]: W"
            
            # --- [MODIFIKASI] BLOK LOGGING ANTI-SPAM ---
            # Hanya cetak jika alasannya berubah dari loop sebelumnya
            if log_reason and log_reason != self._last_log_reason:
                print(log_reason)
                self._last_log_reason = log_reason # Simpan log terakhir
            elif not log_reason and self._last_log_reason:
                # Jika kita beralih ke mode diam (misal: Manual WASD), reset log
    ...
    # ... (Fungsi _handle_gate_traversal_command, _handle_debug_counter, _handle_vision_target_update tetap sama) ...
    ...
    def _handle_manual_control(self, payload):
        ...
        command_str = f"A,{int(servo)},{int(pwm)}\n"
        
        # [MODIFIKASI] Hapus print dari sini.
        # Logika di main_logic_loop akan mencetak "[LOG | MANUAL] Mode Manual GUI (WASD) Aktif."
        # satu kali, yang lebih baik daripada spamming.
        # print(f"[To ESP32 | MANUAL] Mengirim: {command_str.strip()}")
    ...
    def _handle_mode_change(self, payload):
        ...
        if self.current_state["control_mode"] == "MANUAL":
            ...
            # [MODIFIKASI] Cetak perintah netral ini karena ini adalah event satu kali
            command_str = f"A,{int(servo_def)},{int(pwm_stop)}\n"
            log_reason = f"[LOG | MODE] GUI ganti ke MANUAL, kirim netral: {command_str.strip()}"
            print(log_reason) 
            self._last_log_reason = log_reason # Update log anti-spam
            self.serial_handler.send_command(command_str)
