...
class VisionService(QObject):
        ...
        while self.running:
            try:
                ...
                while self.running:
                    ...
                    if not ret:
                        print(
                            f"[Vision Cam-{cam_index}] Peringatan: Gagal membaca frame. Stream mungkin terputus."
                        )
                        break
                    ...
                    processed_frame = (
                        self.process_and_control(frame, is_mode_auto)
                        if apply_detection
                        else frame
                    )
                ...
                if self.running:
                    print(
                        f"[Vision Cam-{cam_index}] Stream terputus, akan mencoba memulai ulang dalam 5 detik."
                    )
                    time.sleep(5)
    ...
    def process_and_control(self, frame, is_mode_auto):
        ...
        current_mission_phase = self.asv_handler.get_current_state().get(
            "mission_phase"
        )
        ...
        if is_mode_auto:
            ...
            if detected_green_boxes or detected_blue_boxes:
                self.handle_photography_mission(
                    frame, detected_green_boxes, detected_blue_boxes
                )
    ...
    def handle_auto_control_dwa(self, red_buoys, green_buoys):
        ...
        if all_buoys:
            target_detection = max(
                all_buoys,
                key=lambda b: (b["xyxy"][2] - b["xyxy"][0])
                * (b["xyxy"][3] - b["xyxy"][1]),
            )
        ...
        obstacles_relative = [
            self._convert_detection_to_relative_coords(b) for b in all_buoys
        ]
    ...
    def validate_and_trigger_investigation(self, poi_data, frame):
        ...
        is_consistent = all(
            cls == first_detection_class for cls in self.recent_detections
        )

        if is_consistent:
            print(
                f"[Vision] VALIDASI BERHASIL: Objek '{first_detection_class}' terdeteksi secara konsisten."
            )
