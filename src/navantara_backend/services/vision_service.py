...
class VisionService:
    ...
    def _validate_buoy_color(self, frame, detection):
        ...
        if (
            "buoy" not in detection.get("class", "")
            and "bola" not in detection.get("class", "")
            and "kotak" not in detection.get("class", "")
        ):
    ...
    def handle_photography_mission(self, current_state, mode="surface"):
        ...
        Menghormati flag camera_swapped agar foto diambil dari kamera fisik yang benar.
        """
        frame_to_use = None
        mission_name = None
        filename_prefix = None
        ...
        # Baca status swap secara thread-safe
        with self.settings_lock:
