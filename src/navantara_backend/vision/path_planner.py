...
def dwa_path_planning(current_state, obstacles, goal_point, config):
    ...
    yaw_rate_resolution = np.radians(dwa_cfg.get("yaw_rate_resolution_dps", 1.0)) # rad/s
    ...
    for v in np.arange(v_window[0], v_window[1], v_resolution):
        for omega in np.arange(omega_window[0], omega_window[1], yaw_rate_resolution):
            ...
            heading_score = to_goal_cost_gain * _calculate_heading_score(trajectory, goal_point)
            clearance_score = obstacle_cost_gain * _calculate_clearance_score(trajectory, obstacles, robot_radius)
