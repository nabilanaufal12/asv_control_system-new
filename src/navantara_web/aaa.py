...
# Variabel global untuk menyimpan frame terakhir dan lock
output_frame = None
lock = threading.Lock()
...
def hasilkan_frame():
    ...
    while True:
        with lock:
            ...
            ret, buffer = cv2.imencode('.webp', output_frame, [cv2.IMWRITE_WEBP_QUALITY, 50])
            if not ret:
                continue
...
@app.route('/video')
def video_feed():
    return Response(hasilkan_frame(),
                    mimetype='multipart/x-mixed-replace; boundary=frame')

if __name__ == '__main__':
    # Buat dan jalankan thread untuk update frame kamera
    thread_kamera = threading.Thread(target=update_frame)
    thread_kamera.daemon = True
    thread_kamera.start()
    
    # Buat dan jalankan thread untuk server