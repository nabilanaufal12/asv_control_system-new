...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        scroll_area_kiri.setMinimumWidth(460)
        ...
        scroll_area_kanan.setMinimumWidth(460)
        ...
        top_stream_bar.setContentsMargins(0, 0, 0, 0)
        top_stream_bar.addStretch()  # Dorong tombol ke pojok kanan
        top_stream_bar.addWidget(self.video_view.invert_button)
        top_stream_bar.addWidget(self.video_view.start_stop_button)
        ...
        splitter_sizes = gui_settings.get("main_splitter_sizes", [480, 800, 480])
    ...
    def connect_signals(self):
        ...
        self.waypoints_panel.counter_action_requested.connect(
            lambda action: self.api_client.send_command(
                "DEBUG_WP_COUNTER", {"action": action}
            )
        )
        ...
        self.settings_panel.send_box_avoidance_config.connect(
            lambda payload: self.api_client.send_command(
                "UPDATE_BOX_AVOIDANCE_CONFIG", payload
            )
        )
