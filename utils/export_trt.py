...
def export_tensorrt():
    ...
    if not os.path.exists(model_path):
        print("[ERROR] PyTorch model not found at: {model_path}")
        return

    print("[1] Loading PyTorch model from: {model_path}")
    ...
    print(
        "             It may take 10 - 20 minutes to complete depending on thermal constraints."
    )
    print("             DO NOT interrupt the process!\n")

    # Perform the export
    exported_path = model.export(format="engine", device=0, half=True, workspace=4)

    print("\n" + "=" * 60)
    print("[3] SUCCESS! Model exported to TensorRT engine.")
