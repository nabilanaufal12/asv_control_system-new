        ...
        if os.path.isfile(path) and item.endswith(".py"):
            if item in entry_points:
                ...
                try:
                    ...
                    warnings = []
                    if "sys.path.append" in content or "os.path.dirname(__file__)" in content:
                        warnings.append("Contains sys.path manipulation which might break if depth is changed.")
                    
                    new_loc = f"tests/{item}" if item.startswith("test_") else f"utils/{item}"
                    moves.append({
                        "file": item,
                        "new_loc": new_loc,
                        "warnings": warnings
