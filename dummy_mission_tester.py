...
def simulate_mission():
    ...
    while True:
        ...
        for target_index in range(len(DUMMY_MISSION_WPS)):
            ...
            print(f"[Simulator] Menuju ke WP {target_index} ({target_wp['lat']}, {target_wp['lon']})")
            
            with state_lock:
                current_telemetry["nav_target_wp_index"] = target_index
                current_telemetry["status"] = f"WAYPOINT NAVIGATION ({target_index+1}/{len(DUMMY_MISSION_WPS)})"
            ...
            for step in range(5):
                ...
                with state_lock:
                    ...
                    new_lat = current_lat + (target_wp["lat"] - current_lat) * 0.2 # 1/5 jarak
                    new_lon = current_lon + (target_wp["lon"] - current_lon) * 0.2 # 1/5 jarak
                    
                    current_telemetry["latitude"] = new_lat
                    current_telemetry["longitude"] = new_lon
                    current_telemetry["heading"] = (current_telemetry["heading"] + 5) % 360
...
def stream_telemetry():
    ...
    return Response(stream_with_context(generate_telemetry()), mimetype='text/event-stream')
