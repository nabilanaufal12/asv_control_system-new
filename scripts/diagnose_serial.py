...
def diagnose_serial_ports():
    ...
    print(
        "1. Bandingkan 'Description' di atas dengan daftar 'auto_connect_descriptors' di 'config/config.json' Anda."
    )
    print(
        "2. Jika ada deskripsi yang cocok tapi koneksi 'AUTO' gagal, pastikan Anda sudah memperbaiki izin (permissions)."
    )
    print(
        "3. Jika tidak ada deskripsi yang cocok, tambahkan deskripsi unik dari perangkat Anda ke dalam daftar di config.json."
    )
    print(
        "4. Sebagai alternatif, Anda bisa memilih 'Device' secara manual (misal: /dev/ttyUSB0) di GUI daripada menggunakan 'AUTO'."
    )
    print("-----------------------------------------")
