...
# --- PERBAIKAN: Menambahkan print untuk debug servo dan motor ---
...
class DetectionThread(QThread):
    ...
    def pairing_score(self, ball1, ball2):
        size1 = (ball1['xyxy'][2] - ball1['xyxy'][0]) * (ball1['xyxy'][3] - ball1['xyxy'][1])
        size2 = (ball2['xyxy'][2] - ball2['xyxy'][0]) * (ball2['xyxy'][3] - ball2['xyxy'][1])
        avg_y = (ball1['center'][1] + ball2['center'][1]) / 2
        return 0.6 * (size1 + size2) + 0.4 * avg_y
    ...
    def run(self):
        ...
        try:
            ...
            if not cap.isOpened():
                print(f"ERROR: Tidak bisa membuka kamera indeks {self.source_idx}")
                return
                ...
                for i, det in enumerate(pred):
                    ...
                    if len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            bbox_data = { 'xyxy': xyxy, 'center': (int((xyxy[0] + xyxy[2]) / 2), int((xyxy[1] + xyxy[3]) / 2)) }
                        ...
                        if detected_red_balls and detected_green_balls:
                            ...
                            for red in detected_red_balls:
                                for green in detected_green_balls:
                                    score = self.pairing_score(red, green)
                                    if score > best_score:
                                        best_score = score
                                        best_pair = (red, green)
                        
                        if best_pair:
                            red_nearest, green_nearest = best_pair
                            x1, y1 = min(red_nearest['xyxy'][0], green_nearest['xyxy'][0]), min(red_nearest['xyxy'][1], green_nearest['xyxy'][1])
                            x2, y2 = max(red_nearest['xyxy'][2], green_nearest['xyxy'][2]), max(red_nearest['xyxy'][3], green_nearest['xyxy'][3])
                            
                            LEBAR_ASLI_OBJEK_CM, PANJANG_FOKUS_PIKSEL = 20.0, 600
                            lebar_objek_piksel = x2 - x1
                            jarak_estimasi_cm = (LEBAR_ASLI_OBJEK_CM * PANJANG_FOKUS_PIKSEL) / lebar_objek_piksel if lebar_objek_piksel > 0 else 0
                            
                            annotator.box_label([x1, y1, x2, y2], f"Jarak: {jarak_estimasi_cm:.1f} cm")

                            batas_posisi_y, TARGET_JARAK_CM = im0.shape[0] * 0.5, 30.0
    ...
    def stop(self):
        print("Memberi sinyal stop ke thread deteksi...")
        self.running = False
